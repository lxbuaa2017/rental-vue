              ...
              <el-card :body-style="{ padding: '0px' }" shadow="hover">
                ...
                <div class="label">
                  <div style="padding: 5px">
                    <el-link :underline="false" style="font-size: 16px" @click="detailShow(list.roomid)">{{list.address}}（{{list.status}}）</el-link>
...
export default {
  ...
  methods: {
    ...
    detailShow (id) {
      let self = this
      this.$axios.get('/api/room/findById?id=' + id).then((res) => {
        console.log(res)
        let state = ''
